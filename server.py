...
def get_system_info_output():
    ...
    def get_network_info():
        ...
        network_info = f"📥 Incoming: {mb_received:.2f} MB\n📤 Outgoing: {mb_sent:.2f} MB\n"
        return network_info

    # Generate the output
    network_info = get_network_info()
    output = "ℹ️ **System Info**\n"
    if public_ip:
        output += f"🌐 **IP Address** `{public_ip}`\n"
        if ip_details:
            output += f"🏙️ **City** `{ip_details.get('city')}`\n"
    ...
    output += f"📶 **Network info:** \n`{network_info}`\n"
    output += f"⏳ **Uptime** `{uptime_str}`\n\n"
    output += f"🖥️ **OS** `{os_info}`\n"
